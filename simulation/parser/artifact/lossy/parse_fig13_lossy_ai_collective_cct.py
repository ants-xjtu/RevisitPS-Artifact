...
def main() -> int:
    parser = argparse.ArgumentParser(description="Parse Figure 13 lossy AI collective CCT JSON.")
    add_common_args(parser)
    args = parser.parse_args()
    if args.manifest is None:
        raise SystemExit("ERROR: --manifest is required")
    select_manifest_history(args.manifest, args.history, args.selected_history, figures={"figure13"}, expected=47)
    stage_parser(
        args.ns3_root,
        args.stage_dir,
