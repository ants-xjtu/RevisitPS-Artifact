...
def main():
    parser = argparse.ArgumentParser(description='Parse FCT results into JSON files, grouping by error rate and timeout mode.')
    ...
    json_dir = os.path.join(file_dir, "json-data-fct-error")
        ...
        for line in f.readlines():
            for topo_prefix in topo2bdp.keys():
                if topo_prefix in line:
                    ...
                    # --- CHANGE: Added timeout_mode to the grouping key ---
                    key = (topo, netload, flow_control, load_type, timeout_mode)
    ...
    for k, v in map_key_to_id.items():
        ...
        plot_group_data = {
            "metadata": {
                ...
                "flow_control": k[2],
                "load_type": k[3],
                "timeout_mode": k[4], # --- CHANGE: Added timeout_mode to metadata ---
            },
        ...
        # --- CHANGE: Updated JSON filename to include timeout mode ---
        json_filename = f"{json_dir}/DATA_TOPO_{k[0]}_LOAD_{k[1]}_FC_{k[2]}_TYPE_{k[3]}_TIMEOUT_{k[4]}.json"
