#!/usr/bin/python3
"""
Parse FCT results into JSON files, one JSON per (topology, flow_control,
buffer_size).

Grouping key:
    (topo, flow_control, buffer_size)

Each output JSON contains one curve per run inside that (topo, fc, bufsz)
bucket -- the varying legend is whatever differs between runs
(lb_mode / cc_mode / recovery / netload / window / rto...).
...
def _worker_parse(task):
    key, entry, fct_file, time_start, time_end, num_buckets = task
    return key, entry, get_steps_from_raw(fct_file, time_start, time_end,
                                          num_buckets=num_buckets)


def iter_history_files(inputs):
    """Yield every .txt history file from a mix of file/dir inputs."""
    for p in inputs:
        if os.path.isdir(p):
...
def main():
    parser = argparse.ArgumentParser(
        description='Parse FCT results into JSON files grouped by '
                    '(topology, flow_control, buffer_size).'
    )
    parser.add_argument('inputs', nargs='+',
                        help='One or more history files or directories of history files.')
    ...
    parser.add_argument('--out-dir', default=None,
                        help='Output directory (default: parser/json-data-fct-per-bufsz/).')
    parser.add_argument('-j', '--jobs', type=int, default=1,
                        help='Parallel workers for FCT file parsing (default: 1).')
    ...
    json_dir = args.out_dir or os.path.join(file_dir, "json-data-fct-per-bufsz/")
    os.makedirs(json_dir, exist_ok=True)
    output_dir = os.path.join(file_dir, "../mix/output")

    # bucket: (topo, flow_control, buffer_size) -> list of entry dicts
    map_key_to_id = defaultdict(list)

    for history_filename in iter_history_files(args.inputs):
        print(f"Reading: {history_filename}")
        with open(history_filename, "r") as f:
            for line in f.readlines():
                for topo_prefix in topo2bdp.keys():
                    if topo_prefix not in line:
                        continue
                    ...
                    # Grouping: one JSON per (topo, flow_control, buffer_size).
                    key = (topo, flow_control, buffer_size)
    ...
    # Collect all (group_key, entry, fct_file) tuples, then parse in parallel.
    def _sort_key(entry):
        return (entry["lb_mode"] or "", entry["cc_mode"] or "",
                entry["recovery"] or "", entry["netload"] or "",
                entry["config_id"])

    tasks = []
    for (topo, flow_control, buffer_size), entries in sorted(map_key_to_id.items()):
        for entry in sorted(entries, key=_sort_key):
            fct_file = f"{output_dir}/{entry['config_id']}/{entry['config_id']}_out_fct.txt"
            if os.path.exists(fct_file):
                tasks.append(((topo, flow_control, buffer_size), entry, fct_file,
                              time_start, time_end, NUM_BUCKETS))

    print(f"Parsing {len(tasks)} FCT files with {args.jobs} worker(s)...")
    parsed_results = defaultdict(list)
    if args.jobs > 1 and tasks:
        with ProcessPoolExecutor(max_workers=args.jobs) as pool:
            for key, entry, result in pool.map(_worker_parse, tasks):
                parsed_results[key].append((entry, result))
    else:
        for task in tasks:
            key, entry, result = _worker_parse(task)
            parsed_results[key].append((entry, result))
    ...
    for (topo, flow_control, buffer_size), entries in sorted(map_key_to_id.items()):
        ...
        for entry, result in parsed_results.get((topo, flow_control, buffer_size), []):
            if not result or not result["avg"]:
                continue
            series_data = {
                "config_id":               entry["config_id"],
                "load_balancing_mode":     entry["lb_mode"],
                "congestion_control_mode": entry["cc_mode"],
        ...
        if not plot_group_data["data_series"]:
            print(f"Skipping group (no data): topo={topo} fc={flow_control} bufsz={buffer_size}")
            continue

        json_filename = (
            f"DATA_TOPO_{sanitize(topo)}"
            f"_FC_{sanitize(flow_control)}"
            f"_BUF_{sanitize(buffer_size)}.json"
        )
        json_path = os.path.join(json_dir, json_filename)
        print(f"Saving {len(plot_group_data['data_series'])} series -> {json_path}")
