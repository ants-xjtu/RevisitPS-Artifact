...
def parse_ooo_stats(filename):
    ...
    Returns:
        dict: A dictionary containing the parsed OOO stats. Reordering distance
              and burst size are converted to packet counts (value/1000 and ceil).
    ...
    # Use defaultdict to aggregate counts for transformed values
            ...
            for line in f:
                ...
                if line.startswith('[OOO Overall Stats]'):
                    ...
                    if current_section == 'overall' and len(parts) == 3:
                        ...
                        count = int(parts[1])
                        # --- MODIFICATION: Convert to packets and aggregate ---
    ...
    # Convert aggregated data to a list of [value, count] and sort
    stats['reordering_distance_packets'] = sorted(dist_agg.items(), key=lambda x: x[0])
    stats['burst_size_packets'] = sorted(burst_agg.items(), key=lambda x: x[0])
...
def main():
    ...
    json_dir = os.path.join(file_dir, "json-data-ooo") # Separate directory for OOO data
    ...
    for k, v_configs in map_key_to_config.items():
        ...
        for entry in v_configs:
            ...
            # Parse the OOO stats file for the current configuration
            result = parse_ooo_stats(ooo_stats_file)

            # Only add data if parsing was successful and returned data
            if result:
                series_data = {
                    ...
                    "ooo_burst_size_packets": result["burst_size_packets"]
                }
