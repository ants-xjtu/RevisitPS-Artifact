...
topo2bdp = {
    ...
    "leaf_spine_L2_S4_100G_OS1": 104000,
     "leaf_spine_L2_S4_100G_OS2": 104000,
...
def get_steps_from_raw(filename, time_start, time_end, num_buckets=20):
    """
    Processes a raw FCT data file to extract FCT statistics.
    This function has been modified to accept 'num_buckets' instead of a fixed step.
    """
    cmd_get_raw = f"cat {filename} | awk '{{ if ($6 > {time_start} && $6+$7 < {time_end}) {{ print $7/1000, $5}} }}'"
    try:
    ...
    for size in sorted(flows_by_size.keys()):
        fcts_for_size = sorted(flows_by_size[size])
        n_flows = len(fcts_for_size)
        if n_flows == 0:
            continue
        buckets = [[] for _ in range(internal_reorder_buckets)]
        for i, fct in enumerate(fcts_for_size):
            buckets[i % internal_reorder_buckets].append(fct)
        for bucket in buckets:
            for fct in bucket:
                reordered_flows.append([fct, size])

    if not reordered_flows:
        return {"avg": [], "p99": [], "size": []}

    aa = [f"{fct} {size}" for fct, size in reordered_flows]
    nn = len(aa)
    ...
    for i in range(num_buckets):
        ...
        current_res.append(fct[-1]) # Tail (worst) FCT
        res.append(current_res)
    # --- END OF MODIFICATION ---

    result = {"avg": [], "p50": [], "p80": [], "p90": [], "p95": [], "p99": [], "p999": [], "tail": [], "size": []}
    for item in res:
        if len(item) > 9:
...
def main():
    ...
    json_dir = os.path.join(file_dir, "json-data-real-fct/")  # You can update the path accordingly
    ...
    for k, v in map_key_to_id.items():
        ...
        for entry in v:
            ...
            if os.path.exists(fct_file):
                ...
                result = get_steps_from_raw(fct_file, time_start, time_end, num_buckets=NUM_BUCKETS)
