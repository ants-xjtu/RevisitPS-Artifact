...
def parse_raw_pfc_log(filename):
    ...
    with open(filename, 'r') as f:
        for line in f:
            line = line.strip()
            if not line or not line.startswith('PFC'): 
                continue
            try:
                parts = line.split(',')
                if len(parts) < 9: 
...
def main():
    parser = argparse.ArgumentParser(description='Parse DOWNLINK PFC incast events and save merged JSON.')
    parser.add_argument('history_file', type=str, help='Path to the history file to process.')
    args = parser.parse_args()

    file_dir = getFilePath()
    json_dir = os.path.join(file_dir, "json-data-pfc-incast-workload")
    ...
    print("NOTE: Analysis is filtered to include DOWNLINK PFC events ONLY (Lossless only).")
        ...
        for line in f:
            ...
            try:
                ...
                if flow_control == "Lossless":
                    key = (topo, netload, flow_control)  # merge all workloads
                    entry_details = {
                        "config_id": config_id,
                        "lb_mode": lb_mode_str,
    ...
    # --- Step 2: Process all groups and merge different workloads into a single JSON ---
    all_workloads_data = {
        "metadata": {
            "description": "Merged PFC Incast data for all workloads (DOWNLINK only, Lossless)."
        },
        "data_series": []
    }
    ...
    for k, v_configs in map_key_to_config.items():
        for entry in v_configs:
            ...
            all_workloads_data["data_series"].append(series_data)

    # --- Step 3: Save the merged JSON file ---
    merged_json_file = os.path.join(json_dir, "all_workloads_pfc_incast.json")
    print(f"Saving merged PFC Incast data to: {merged_json_file}")
    with open(merged_json_file, 'w') as f:
        json.dump(all_workloads_data, f, indent=4)

    print("\n✅ All workloads merged into a single JSON successfully!")
