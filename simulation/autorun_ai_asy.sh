            ...
            # CC 不再循環 — 請在每個 run_experiment_group 調用中顯式指定 CC (dcqcn/hpcc/timely/dcqcn_lane/...)
                for ai_nodes_per_group in "${AI_NODES_PER_GROUP[@]}"; do
                    ...
                    if [[ "$cdf" == "AlltoallV" ]]; then
                        ...
                        run_experiment_group "$topo" "$actual_load" "$FIXED_ERROR_RATE" "$cdf" dcqcn 1 0 ar 0 "$default_win_size" 4000 4000 "$BUFFER_SIZE" "$effective_n" rps adaptive drillgroup sglb
                        run_experiment_group "$topo" "$actual_load" "$FIXED_ERROR_RATE" "$cdf" none 1 0 ar 0 "$default_win_size" 4000 4000 "$BUFFER_SIZE" "$effective_n" drillgroup sglb
                         # # Lossy
                        ...
                        run_experiment_group "$topo" "$actual_load" "$FIXED_ERROR_RATE" "$cdf" dcqcn 0 2 ar 0 "$default_win_size" "$RTO_HIGH" "$RTO_LOW" "$BUFFER_SIZE" "$effective_n" rps adaptive drillgroup sglb
                        # run_experiment_group "$topo" "$actual_load" "$FIXED_ERROR_RATE" "$cdf" dcqcn 0 2 ar 2 "$default_win_size" "$RTO_HIGH" "$RTO_LOW" "$BUFFER_SIZE" "$effective_n" adaptive drillgroup sglb
                        run_experiment_group "$topo" "$actual_load" "$FIXED_ERROR_RATE" "$cdf" dcqcn 0 1 ar 2 "$default_win_size" "$RTO_HIGH" "$RTO_LOW" "$BUFFER_SIZE" "$effective_n" rps adaptive drillgroup sglb
                    ...
                    # (N=effective_n=8, per-flow message_size ≈ 21.4 MB, 每节点收 150 MB)
                        ...
                        run_experiment_group "$topo" "$actual_load" "$FIXED_ERROR_RATE" "$cdf" dcqcn 0 1 noar 0 "$default_win_size" "$RTO_HIGH" "$RTO_LOW" "$BUFFER_SIZE" "$effective_n" fecmp conga conweave
                        run_experiment_group "$topo" "$actual_load" "$FIXED_ERROR_RATE" "$cdf" dcqcn 0 1 ar 0 "$default_win_size" "$RTO_HIGH" "$RTO_LOW" "$BUFFER_SIZE" "$effective_n" rps adaptive drillgroup sglb
                        run_experiment_group "$topo" "$actual_load" "$FIXED_ERROR_RATE" "$cdf" dcqcn 0 2 ar 0 "$default_win_size" "$RTO_HIGH" "$RTO_LOW" "$BUFFER_SIZE" "$effective_n" rps adaptive drillgroup sglb
                        run_experiment_group "$topo" "$actual_load" "$FIXED_ERROR_RATE" "$cdf" none 0 1 ar 0 "$default_win_size" "$RTO_HIGH" "$RTO_LOW" "$BUFFER_SIZE" "$effective_n" drillgroup sglb
                        run_experiment_group "$topo" "$actual_load" "$FIXED_ERROR_RATE" "$cdf" none 0 2 ar 0 "$default_win_size" "$RTO_HIGH" "$RTO_LOW" "$BUFFER_SIZE" "$effective_n" drillgroup sglb
                        ...
                        run_experiment_group "$topo" "$actual_load" "$FIXED_ERROR_RATE" "$cdf" dcqcn 1 0 noar 0 "$default_win_size" 4000 4000 "$BUFFER_SIZE" "$effective_n" fecmp conga conweave
                        run_experiment_group "$topo" "$actual_load" "$FIXED_ERROR_RATE" "$cdf" dcqcn 1 0 ar 0 "$default_win_size" 4000 4000 "$BUFFER_SIZE" "$effective_n" rps adaptive drillgroup sglb
                        run_experiment_group "$topo" "$actual_load" "$FIXED_ERROR_RATE" "$cdf" none 1 0 ar 0 "$default_win_size" 4000 4000 "$BUFFER_SIZE" "$effective_n" rps adaptive drillgroup sglb
                        # run_experiment_group "$topo" "$actual_load" "$FIXED_ERROR_RATE" "$cdf" dcqcn 1 0 ar 0 "$default_win_size" "$RTO_HIGH" "$RTO_LOW" "$BUFFER_SIZE" "$effective_n" rps drill adaptive
                        # run_experiment_group "$topo" "$actual_load" "$FIXED_ERROR_RATE" "$cdf" dcqcn 1 0 ar 0 512000 "$RTO_HIGH" "$RTO_LOW" "$BUFFER_SIZE" "$effective_n" rps drill adaptivea
                    ...
                    # (N=effective_n=8, per-flow message_size ≈ 10.7 MB, 每节点收 150 MB)
                        ...
                        # run_experiment_group "$topo" "$actual_load" "$FIXED_ERROR_RATE" "$cdf" dcqcn 0 1 ar 0 "$default_win_size" "$RTO_HIGH" "$RTO_LOW" "$BUFFER_SIZE" "$effective_n" rps adaptive
                        run_experiment_group "$topo" "$actual_load" "$FIXED_ERROR_RATE" "$cdf" dcqcn 0 1 ar 0 512000 "$RTO_HIGH" "$RTO_LOW" "$BUFFER_SIZE" "$effective_n" rps adaptive drillgroup sglb
                        run_experiment_group "$topo" "$actual_load" "$FIXED_ERROR_RATE" "$cdf" dcqcn 0 2 ar 0 512000 "$RTO_HIGH" "$RTO_LOW" "$BUFFER_SIZE" "$effective_n" rps adaptive drillgroup sglb
                        run_experiment_group "$topo" "$actual_load" "$FIXED_ERROR_RATE" "$cdf" none 0 1 ar 0 512000 "$RTO_HIGH" "$RTO_LOW" "$BUFFER_SIZE" "$effective_n" drillgroup sglb
                        run_experiment_group "$topo" "$actual_load" "$FIXED_ERROR_RATE" "$cdf" none 0 2 ar 0 512000 "$RTO_HIGH" "$RTO_LOW" "$BUFFER_SIZE" "$effective_n" drillgroup sglb
                        # run_experiment_group "$topo" "$actual_load" "$FIXED_ERROR_RATE" "$cdf" dcqcn 0 1 ar 0 0 "$RTO_HIGH" "$RTO_LOW" "$BUFFER_SIZE" "$effective_n" rps drill adaptive
                        ...
                        run_experiment_group "$topo" "$actual_load" "$FIXED_ERROR_RATE" "$cdf" dcqcn 1 0 ar 0 512000  4000 4000 "$BUFFER_SIZE" "$effective_n" rps adaptive drillgroup sglb
                        ...
                        run_experiment_group "$topo" "$actual_load" "$FIXED_ERROR_RATE" "$cdf" none 1 0 ar 0 512000  4000 4000 "$BUFFER_SIZE" "$effective_n" drillgroup sglb
                    fi
