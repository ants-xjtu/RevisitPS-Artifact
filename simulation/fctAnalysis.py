...
if __name__=="__main__":
	...
	with open(output_fct_summary, "w") as outfile_fct_summary:
		...
		# Reorder flows by size to make FCT distribution uniform across buckets
		raw_flows = [[float(x.split(" ")[0]), int(x.split(" ")[1])] for x in aa]
		...
		for size in sorted(flows_by_size.keys()):
			...
			for idx, slowdown in enumerate(slowdowns_for_size):
				buckets[idx % internal_reorder_buckets].append(slowdown)
			for bucket in buckets:
				for slowdown in bucket:
					reordered_flows.append([slowdown, size])

		# Convert back to string format for compatibility with existing code
		aa = ["{} {}".format(slowdown, size) for slowdown, size in reordered_flows]
		nn = len(aa)
		...
		# Reorder flows by size to make FCT distribution uniform across buckets
		raw_flows_abs = [[float(x.split(" ")[0]), int(x.split(" ")[1])] for x in a]
		flows_by_size_abs = defaultdict(list)
		for fct_val, size in raw_flows_abs:
			flows_by_size_abs[size].append(fct_val)

		reordered_flows_abs = []
		internal_reorder_buckets = 100
		for size in sorted(flows_by_size_abs.keys()):
			fcts_for_size = sorted(flows_by_size_abs[size])
			n_flows = len(fcts_for_size)
			if n_flows == 0:
				continue
			buckets = [[] for _ in range(internal_reorder_buckets)]
			for idx, fct_val in enumerate(fcts_for_size):
				buckets[idx % internal_reorder_buckets].append(fct_val)
			for bucket in buckets:
				for fct_val in bucket:
					reordered_flows_abs.append([fct_val, size])

		# Convert back to string format for compatibility with existing code
		a = ["{} {}".format(fct_val, size) for fct_val, size in reordered_flows_abs]
		n = len(a)
