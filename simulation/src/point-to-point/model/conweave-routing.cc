...
uint64_t ConWeaveRouting::m_nInvalidTailTime = 0;
...
uint64_t ConWeaveRouting::CalculateTimegapAtTx(uint64_t tailTime, Time phase0TxTime,
                                               bool *inconsistentTailTime) {
    uint64_t phase0TxNs = phase0TxTime.GetNanoSeconds();
    bool inconsistent = tailTime < phase0TxNs;
    if (inconsistentTailTime != 0) {
        *inconsistentTailTime = inconsistent;
    }
    return inconsistent ? 0 : tailTime - phase0TxNs;
}
         ...
         ***************************/
            ...
            if (foundConWeaveDataTag) {  // DATA
                ...
                if (rx_md.flagPhase0Cache) { /* phase0-timestamp is available */
                    bool invalidTailTime = false;
                    rx_md.timegapAtTx =
                        CalculateTimegapAtTx(rx_md.tailTime, rx_md.phase0TxTime, &invalidTailTime);

                    /** DEBUGGING: */
                    if (rx_md.pkt_phase == 1 || rx_md.pkt_flagData == ConWeaveDataTag::TAIL) {
                        if (invalidTailTime) {
                            m_nInvalidTailTime += 1;
                            SLB_LOG(PARSE_FIVE_TUPLE(ch)
                            ...
                            std::cout << "** CONWEAVE WARNING - Though this pkt has tailTime, the "
                                         "tailTime is before Phase0-TxTime. "
                                         "Continuing with timegapAtTx=0"
                                      << std::endl;
                            std::cout << "** tailTime:" << rx_md.tailTime
                                      << ", Phase0TxTime:" << rx_md.phase0TxTime.GetNanoSeconds()
                                      << ", invalidTailTimeCount:" << m_nInvalidTailTime
                                      << std::endl;
                        }
                    }
