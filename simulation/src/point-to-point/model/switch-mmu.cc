...
void SwitchMmu::InitSwitch(void) {
    ...
    m_pg_shared_alpha_cell_off_diff = 3 * MTU;
    m_port_shared_alpha_cell_off_diff = 16;

    m_xoffset = 3 * MTU;
    // std::cout << "Ingress alpha:" << m_pg_shared_alpha_cell << std::endl;
    // std::cout << "Egress alpha: " << m_pg_shared_alpha_cell_egress << std::endl;
    // std::cout << "PFC_endabled" << m_PFCenabled << std::endl;
}
...
uint32_t SwitchMmu::DynamicThreshold(uint32_t port, uint32_t qIndex) {
    if(m_usedIngressSPBytes[GetIngressSP(port, qIndex)] >= m_buffer_cell_limit_sp) {
        return 0;
    }
    else
    {
        return (uint32_t)(m_pg_shared_alpha_cell * (m_buffer_cell_limit_sp -  
...
bool SwitchMmu::CheckShouldPause(uint32_t port, uint32_t qIndex) {
    return !paused[port][qIndex] && ((m_usedIngressPGHeadroomBytes[port][qIndex] > 0) || (m_usedIngressPGBytes[port][qIndex] > DynamicThreshold(port, qIndex)));
}

bool SwitchMmu::CheckShouldResume(uint32_t port, uint32_t qIndex) {
    if(!paused[port][qIndex]) return false;
    bool shouldResume = false;
    if(m_usedIngressPGHeadroomBytes[port][qIndex] == 0 && (m_usedIngressPGBytes[port][qIndex] + m_xoffset<= DynamicThreshold(port, qIndex))) {
        shouldResume = true;
    }
    return shouldResume;
}
...
void SwitchMmu::SetDynamicThreshold(bool v) {
    m_dynamicth = v;
    // InitSwitch();
    // std::cout << "SwitchMmu: Dynamic Threshold is set to " << m_dynamicth << std::endl;
    return;
}
