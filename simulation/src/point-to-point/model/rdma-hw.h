...
/*modification begin*/
// New struct for per-destination DCQCN state.
// It contains all the state variables needed for the DCQCN algorithm.
struct DestDcqcnState : public SimpleRefCount<DestDcqcnState> {
    DataRate m_rate;
    DataRate m_targetRate;
    double m_alpha{0.0};
    bool m_alpha_cnp_arrived{false};
    bool m_decrease_cnp_arrived{false};
    bool m_first_cnp{true};
    uint32_t m_rpTimeStage{0};
    EventId m_eventUpdateAlpha;
    EventId m_eventDecreaseRate;
    EventId m_rpTimer;

    // List to keep track of all QPs sharing this state.
    std::list<Ptr<RdmaQueuePair>> qps;
    uint32_t m_activeQps;

    DestDcqcnState() = default;
};

// Per-Lane DCQCN state (for mode 5)
// Each lane runs one QP at a time with its own DCQCN control
struct LaneDcqcnState : public SimpleRefCount<LaneDcqcnState> {
    uint32_t m_laneId;              // Lane ID within the lane group
    DataRate m_rate;
    DataRate m_targetRate;
    double m_alpha{0.0};
    bool m_alpha_cnp_arrived{false};
    ...
    // Lane scheduling: only one QP runs at a time
    Ptr<RdmaQueuePair> m_runningQp;               // Currently running QP (null if idle)
    std::list<Ptr<RdmaQueuePair>> m_waitingQueue; // Waiting QPs (using list for easy removal)

    LaneDcqcnState() : m_laneId(0), m_runningQp(nullptr) {}
};

// Lane group for a source-destination pair (for mode 5)
struct LaneGroup : public SimpleRefCount<LaneGroup> {
    // uint32_t m_sip;                              // Source IP
    uint32_t m_dip;                              // Destination IP
    uint32_t m_numLanes;                         // Number of lanes
    std::vector<Ptr<LaneDcqcnState>> m_lanes;    // Array of lanes
    uint32_t m_nextLaneIdx{0};                   // Round-robin index for lane assignment

    LaneGroup() : m_dip(0), m_numLanes(0) {}
};
/*modification end*/
...
class RdmaHw : public Object {
   public:
    ...
    //Notify Ideal drop
    void NotifyPacketDrop(uint32_t dip, uint16_t sport, uint16_t dport, uint16_t pg, uint32_t seq, uint32_t payloadSize);
    ...
    /* trace throughput and good throughput*/
    uint64_t m_accSentBytes;  // accumulated sent bytes
    uint64_t m_accAckedBytes;  // accumulated acked bytes
    uint64_t m_accUnackedBytes; // accumulated unacked bytes
    ...
    // Reset Mellanox's DCQCN state on timeout
    void ResetDcqcnStateOnTimeout(Ptr<RdmaQueuePair> qp);
    ...
    /*modification begin*/
    /******************************
     * Mellanox's version of DCQCN(per destination some use the DCQCN)
     *****************************/
    // Map from destination IP to its shared DCQCN state
    std::unordered_map<uint32_t, Ptr<DestDcqcnState>> m_destDcqcnStateMap;

    // --- New Per-Destination DCQCN Functions (for mode 2) ---
    // A complete, separate set of functions for the per-destination logic.
    void cnp_received_mlx_Dest(Ptr<RdmaQueuePair> q);
    void UpdateAlphaMlx_Dest(Ptr<DestDcqcnState> destState);
    void ScheduleUpdateAlphaMlx_Dest(Ptr<DestDcqcnState> destState);
    void CheckRateDecreaseMlx_Dest(Ptr<DestDcqcnState> destState);
    void ScheduleDecreaseRateMlx_Dest(Ptr<DestDcqcnState> destState, uint32_t delta);
    void RateIncEventTimerMlx_Dest(Ptr<DestDcqcnState> destState);
    void RateIncEventMlx_Dest(Ptr<DestDcqcnState> destState);
    void FastRecoveryMlx_Dest(Ptr<DestDcqcnState> destState);
    void ActiveIncreaseMlx_Dest(Ptr<DestDcqcnState> destState);
    void HyperIncreaseMlx_Dest(Ptr<DestDcqcnState> destState);

    /******************************
     * Per-Lane DCQCN (mode 5)
     *****************************/
    uint32_t m_lanesPerDestination;  // Number of lanes per src-dst pair

    // Map from (sip << 32 | dip) to LaneGroup
    std::unordered_map<uint64_t, Ptr<LaneGroup>> m_laneGroupMap;

    // Helper function to wake up the next QP in a lane's waiting queue
    void WakeupNextQpInLane(Ptr<LaneDcqcnState> laneState);

    // Per-Lane DCQCN control functions
    void cnp_received_mlx_Lane(Ptr<RdmaQueuePair> q);
    void UpdateAlphaMlx_Lane(Ptr<LaneDcqcnState> laneState);
    void ScheduleUpdateAlphaMlx_Lane(Ptr<LaneDcqcnState> laneState);
    void CheckRateDecreaseMlx_Lane(Ptr<LaneDcqcnState> laneState);
    void ScheduleDecreaseRateMlx_Lane(Ptr<LaneDcqcnState> laneState, uint32_t delta);
    void RateIncEventTimerMlx_Lane(Ptr<LaneDcqcnState> laneState);
    void RateIncEventMlx_Lane(Ptr<LaneDcqcnState> laneState);
    void FastRecoveryMlx_Lane(Ptr<LaneDcqcnState> laneState);
    void ActiveIncreaseMlx_Lane(Ptr<LaneDcqcnState> laneState);
    void HyperIncreaseMlx_Lane(Ptr<LaneDcqcnState> laneState);
    /*modification end*/
     ...
     * Adaptive Routing
     *********************/
    bool m_adaptiveRouting;

    /**********************
     * Timout slow restart
     *********************/
    uint32_t m_timeoutSlowStartMode;  // 0: no slow start, 1: min slow start, 2: 22 slow start
     ...
     * Trace
     *********************/
    TracedCallback<Ptr<RdmaQueuePair> > m_traceRetransmitStart;  // trace source for retransmit start
};
