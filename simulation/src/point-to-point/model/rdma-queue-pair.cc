...
RdmaQueuePair::RdmaQueuePair(uint16_t pg, Ipv4Address _sip, Ipv4Address _dip, uint16_t _sport,
    ...
    ideal.m_enabled = false;

    m_timeout = MilliSeconds(4);

    m_timeoutRetransmitCount = 0;
    m_destDcqcnState = nullptr;
    m_laneDcqcnState = nullptr;
    m_isActive = true;  // Initialize as active
}
...
uint64_t RdmaQueuePair::GetBytesLeft() {
    ...
    uint64_t remainingNewBytes = m_size >= snd_nxt ? m_size - snd_nxt : 0;

    // ======================== DCP 逻辑补充 START ========================
    if (dcp.m_enabled) {
        if (remainingNewBytes == 0 && !dcp.retransmissionQueue.empty()) {
            return 1000;
        }
    }  
    // ======================== DCP 逻辑补充 END ==========================
    // ======================== Ideal 逻辑补充 START =======================
    if (ideal.m_enabled) {
        if (remainingNewBytes == 0 && !ideal.droppedPackets.empty()) {
            return 1000;
        }
    }
    // ======================== Ideal 逻辑补充 END =========================

    return remainingNewBytes;
}
...
void RdmaQueuePair::Acknowledge(uint64_t ack) {
    if (ack > snd_una) {
        snd_una = ack;
        ClearArDropsUpTo(ack);
    }
}

void RdmaQueuePair::RecordArDrop(uint32_t seq) {
    if (seq < snd_una || seq >= snd_nxt) {
        return;
    }
    if (!ar.hasOutstandingDrop || seq > ar.highestOutstandingDropSeq) {
        ar.highestOutstandingDropSeq = seq;
    }
    ar.hasOutstandingDrop = true;
}

void RdmaQueuePair::ClearArDropsUpTo(uint64_t ack) {
    if (ar.hasOutstandingDrop && ack > ar.highestOutstandingDropSeq) {
        ar.hasOutstandingDrop = false;
        ar.highestOutstandingDropSeq = 0;
    }
}

bool RdmaQueuePair::HasOutstandingArDrop() const {
    return ar.hasOutstandingDrop;
}

void RdmaQueuePair::RecordArTimeoutRetransmission() {
    if (HasOutstandingArDrop()) {
        stat.arRetransmissionsWithDrop++;
        Settings::ar_retransmissions_with_drop++;
        ar.hasOutstandingDrop = false;
        ar.highestOutstandingDropSeq = 0;
    } else {
        stat.arSpuriousRetransmissions++;
...
/*modification begin*/
void RecvBlockTracker::Insert(uint32_t seq, uint32_t size) {
    uint32_t start = seq;
    uint32_t end = seq + size;

    // std::cout << "[Insert] seq=" << seq << " size=" << size 
    //           << " -> interval=[" << start << "," << end << ")\n";

    auto it = m_blocks.lower_bound({start, start});
    if (it != m_blocks.begin()) {
        --it;
    }

    std::vector<decltype(it)> to_erase;
    uint32_t new_start = start, new_end = end;

    for (; it != m_blocks.end(); ++it) {
        auto [s, e] = *it;
        if (e < new_start) continue;
        if (s > new_end) break;

        new_start = std::min(new_start, s);
        new_end = std::max(new_end, e);
    ...
    m_blocks.insert({new_start, new_end});

    UpdateStats();
}

void RecvBlockTracker::UpdateStats() {
    m_total_bytes_received = 0;
    for (const auto& block : m_blocks) {
        m_total_bytes_received += (block.second - block.first);
    }

    m_in_order_bytes = GetNextExpectedSeq();

    // std::cout << "[UpdateStats] total_bytes=" << m_total_bytes_received 
    //           << " in_order=" << m_in_order_bytes << " out_order=" << (m_total_bytes_received - m_in_order_bytes) << "\n";
}

uint32_t RecvBlockTracker::GetNextExpectedSeq() const {
    if (m_blocks.empty()) return 0;

    auto it = m_blocks.begin();
    if (it->first == 0) {
        return it->second;  // 完整有序收到 [0, second)
    } else {
        return 0;  // 前面还没收到
    }
}

void RecvBlockTracker::Clear() {
    m_blocks.clear();
    m_total_bytes_received = 0;
    m_in_order_bytes = 0;
}
