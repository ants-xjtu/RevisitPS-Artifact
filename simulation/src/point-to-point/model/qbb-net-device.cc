...
int RdmaEgressQueue::GetNextQindex(bool paused[]) {
    ...
    for (qIndex = 1; qIndex <= fcount; qIndex++) {
        ...
        // modified begin
        bool has_dcp_retransmit = (qp->dcp.m_enabled && !qp->dcp.retransmissionQueue.empty());
        bool cond_window_allowed =
            ((!qp->IsWinBound() || has_dcp_retransmit) &&
            (!qp->irn.m_enabled || qp->CanIrnTransmit(m_mtu)));
        // modified end
        // bool cond_window_allowed =
...
void QbbNetDevice::Resume(unsigned qIndex) {
    ...
    if(m_paused[qIndex]) m_tracePfc(0);
...
void QbbNetDevice::Receive(Ptr<Packet> packet) {
    ...
    if (ch.l3Prot == 0xFE) {  // PFC
        ...
        if (ch.pfc.time > 0) {
            if(!m_paused[qIndex]) m_tracePfc(1);
