...
class SwitchMmu : public Object {
   public:
    ...
    void SetEgressAlpha(double v) {
        m_pg_shared_alpha_cell_egress = v;
    }
    void SetIngressAlpha(double v){
        m_pg_shared_alpha_cell = v; 
    }

    void SetPFCEnabled(bool v) {
        m_PFCenabled = v;
    }

    uint32_t DynamicThreshold(uint32_t port, uint32_t qIndex);
