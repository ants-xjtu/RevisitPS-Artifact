...
# ==============================================================================
#                 AI Collectives Experimentation & Management Script (400G)
# ==============================================================================
#
# Features:
...
#   - [NEW] 自动根据拓扑设置窗口大小 (fat-tree 400G: 606000, leaf-spine 400G: 404000)
...
TOPOLOGIES=(
    ...
    # --- Asymmetric leaf-spine topologies (from autorun_asy.sh, 100G) ---
    # NOTE: These are 100G topologies; uncomment only if you switch NIC_BW to 100
...
RUNTIME="0.02"  # 400G 对应 0.02 (100G=0.05, 200G=0.04, 400G=0.02)
FLOWGEN="src"
...
cecho "BLUE" "=================================================="
cecho "BLUE" "   Starting submission of AI collective communication experiment tasks (400G)"
cecho "BLUE" "=================================================="
...
cecho "YELLOW" "NIC Bandwidth: ${NIC_BW}Gbps"
cecho "YELLOW" "----------------------------------"
...
for topo in "${TOPOLOGIES[@]}"; do
    ...
    default_win_size=0
    if [[ "$topo" == *"fat"* ]]; then
        default_win_size=606000   # fat-tree 400G
    elif [[ "$topo" == *"leaf_spine"* ]] || [[ "$topo" == *"leafspine"* ]]; then
        default_win_size=404000   # leaf-spine 400G
    fi
                ...
                for ai_nodes_per_group in "${AI_NODES_PER_GROUP[@]}"; do
                  for buffer_size in "${BUFFER_SIZES[@]}"; do
                    ...
                    # (N=effective_n=8, per-flow message_size ≈ 10.7 MB, 每节点收 150 MB)
                        ...
                        # --- Scenario 1: IRN RDMA (pfc=0, irn=1) ---
                        run_experiment_group "$topo" "$actual_load" "$FIXED_ERROR_RATE" "$cdf" dcqcn 0 1 noar 0 1024000 "$RTO_HIGH" "$RTO_LOW" "$buffer_size" "$effective_n" fecmp conga conweave
                        # run_experiment_group "$topo" "$actual_load" "$FIXED_ERROR_RATE" "$cdf" dcqcn 0 1 ar 0 "$default_win_size" "$RTO_HIGH" "$RTO_LOW" "$buffer_size" "$effective_n" rps drill adaptive
                        run_experiment_group "$topo" "$actual_load" "$FIXED_ERROR_RATE" "$cdf" dcqcn 0 1 ar 0 1024000 "$RTO_HIGH" "$RTO_LOW" "$buffer_size" "$effective_n" rps drill adaptive
                        run_experiment_group "$topo" "$actual_load" "$FIXED_ERROR_RATE" "$cdf" dcqcn 0 2 ar 0 1024000 "$RTO_HIGH" "$RTO_LOW" "$buffer_size" "$effective_n" adaptive
                        # run_experiment_group "$topo" "$actual_load" "$FIXED_ERROR_RATE" "$cdf" none 0 1 ar 0 "$default_win_size" "$RTO_HIGH" "$RTO_LOW" "$buffer_size" "$effective_n" rps drill adaptive
                        run_experiment_group "$topo" "$actual_load" "$FIXED_ERROR_RATE" "$cdf" none 0 1 ar 0 1024000 "$RTO_HIGH" "$RTO_LOW" "$buffer_size" "$effective_n" rps drill adaptive
                        run_experiment_group "$topo" "$actual_load" "$FIXED_ERROR_RATE" "$cdf" none 0 2 ar 0 1024000 "$RTO_HIGH" "$RTO_LOW" "$buffer_size" "$effective_n" adaptive
                        # run_experiment_group "$topo" "$actual_load" "$FIXED_ERROR_RATE" "$cdf" dcqcn 0 1 ar 0 0 "$RTO_HIGH" "$RTO_LOW" "$buffer_size" "$effective_n" rps drill adaptive

                        # --- Scenario 2: Lossless RDMA (pfc=1, irn=0) ---
                        run_experiment_group "$topo" "$actual_load" "$FIXED_ERROR_RATE" "$cdf" dcqcn 1 0 noar 0 1024000 4000 4000 "$buffer_size" "$effective_n" fecmp conga conweave
                        # run_experiment_group "$topo" "$actual_load" "$FIXED_ERROR_RATE" "$cdf" dcqcn 1 0 ar 0 "$default_win_size" 4000 4000 "$buffer_size" "$effective_n" rps drill adaptive
                        run_experiment_group "$topo" "$actual_load" "$FIXED_ERROR_RATE" "$cdf" dcqcn 1 0 ar 0 1024000 4000 4000 "$buffer_size" "$effective_n" rps drill adaptive
                        # run_experiment_group "$topo" "$actual_load" "$FIXED_ERROR_RATE" "$cdf" none 1 0 ar 0 "$default_win_size" 4000 4000 "$buffer_size" "$effective_n" rps drill adaptive
                        run_experiment_group "$topo" "$actual_load" "$FIXED_ERROR_RATE" "$cdf" none 1 0 ar 0 1024000 4000 4000 "$buffer_size" "$effective_n" rps drill adaptive
                    fi
                  done    # buffer_size loop
