...
def main():
        ...
        for line in f.readlines():
            for topo_prefix in topo2bdp.keys():
                if topo_prefix in line:
                    ...
                    plot_label = f"{lb_mode_str}+{recovery_label}+TMT:{timeout_mode}+Win:{window_size}"
                    
                    flow_control = "Lossless" if pfc == 1 else "Lossy"
                    key = (topo, netload, flow_control, load_type, error_rate, window_size, timeout_mode)
                    map_key_to_id.setdefault(key, []).append([config_id, plot_label])
                    break
    ...
    for k, v in map_key_to_id.items():
        ...
        for tgt_lbmode in lbmode_order:
            for vv in v:
                config_id, full_label = vv
                if full_label.startswith(tgt_lbmode):
                    ...
                    if os.path.exists(fct_slowdown):
                        ...
                        if result and result["avg"]:
                            ax.plot(xvals, result["avg"], markersize=1.0, linewidth=3.0, label=full_label)
                ...
                if full_label.startswith(tgt_lbmode):
                    ...
                    if os.path.exists(fct_slowdown):
                        ...
                        if result and result["p99"]:
                            ax.plot(xvals, result["p99"], markersize=1.0, linewidth=3.0, label=full_label)
