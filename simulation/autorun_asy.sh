...
TOPOLOGIES=(
    ...
    # "leafspine_L8_S16_100G_AsymDelay10pct_5us_OS1"
    # "leafspine_L8_S16_100G_AsymDelay10pct_10us_OS1"
            ...
            for cdf in "${CDFS[@]}"; do
                for cc in "${CCS[@]}"; do
                    for buffer_size in "${BUFFER_SIZES[@]}"; do  # <--- 新增：buffer size 循環
                        ...
                        # big switch test
                        # #Lossy
                        # run_experiment_group "$topo" "$load" "$err_rate" "$cdf" "$cc" 0 1 noar 0 "$default_win_size" 320 320 "$buffer_size" conweave
                        # run_experiment_group "$topo" "$load" "$err_rate" "$cdf" "$cc" 0 1 noar 0 "$default_win_size" 320 320 "$buffer_size"  fecmp letflow conga conweave
                        # run_experiment_group "$topo" "$load" "$err_rate" "$cdf" "$cc" 0 1 ar   2 "$default_win_size" 320 320 "$buffer_size"  adaptive weightadaptive
                        # run_experiment_group "$topo" "$load" "$err_rate" "$cdf" "$cc" 0 2 ar   0 "$default_win_size" 320 320 "$buffer_size"  adaptive weightadaptive
                        # run_experiment_group "$topo" "$load" "$err_rate" "$cdf" "$cc" 0 1 ar   1 "$default_win_size" 320 320 "$buffer_size"  adaptive weightadaptive
                        # run_experiment_group "$topo" "$load" "$err_rate" "$cdf" "$cc" 0 2 ar   1 "$default_win_size" 320 320 "$buffer_size"  adaptive weightadaptive
                        # run_experiment_group "$topo" "$load" "$err_rate" "$cdf" "$cc" 0 1 noar 0 "$default_win_size" 320 320 "$buffer_size"  fecmp conweave
                        # run_experiment_group "$topo" "$load" "$err_rate" "$cdf" "$cc" 0 1 ar   2 "$default_win_size" 320 320 "$buffer_size" rps adaptive  weightadaptive drillgroup sglb
                        # run_experiment_group "$topo" "$load" "$err_rate" "$cdf" "$cc" 0 1 ar   2 "$default_win_size" 320 320 "$buffer_size" drillgroup
                        run_experiment_group "$topo" "$load" "$err_rate" "$cdf" "$cc" 0 1 ar   2 "$default_win_size" 320 320 "$buffer_size" drillgroup  sglb
                        # run_experiment_group "$topo" "$load" "$err_rate" "$cdf" "$cc" 0 2 ar   0 "$default_win_size" 320 100 "$buffer_size" rps adaptive weightadaptive drillgroup sglb
                        run_experiment_group "$topo" "$load" "$err_rate" "$cdf" "$cc" 0 2 ar   0 "$default_win_size" 320 100 "$buffer_size"  rps adaptive
                        run_experiment_group "$topo" "$load" "$err_rate" "$cdf" "$cc" 0 2 ar   2 "$default_win_size" 320 100 "$buffer_size"  rps adaptive
                        # run_experiment_group "$topo" "$load" "$err_rate" "$cdf" "$cc" 0 2 ar   1 "$default_win_size" 320 100 "$buffer_size"  adaptive
                        # run_experiment_group "$topo" "$load" "$err_rate" "$cdf" "$cc" 0 1 noar 0 "$default_win_size" 320 100  fecmp
                        # run_experiment_group "$topo" "$load" "$err_rate" "$cdf" "$cc" 0 1 ar   2 "$default_win_size"   320 100 adaptive
                        # run_experiment_group "$topo" "$load" "$err_rate" "$cdf" "$cc" 0 1 ar   1 "$default_win_size"   320 100 adaptive

                        # # #Lossless
                        # run_experiment_group "$topo" "$load" "$err_rate" "$cdf" "$cc" 1 0 noar 0 "$default_win_size" 4000 4000 "$buffer_size"  fecmp conweave
                        # run_experiment_group "$topo" "$load" "$err_rate" "$cdf" "$cc" 1 0 ar   0 "$default_win_size" 4000 4000 "$buffer_size"  rps adaptive weightadaptive drillgroup sglb
                        # run_experiment_group "$topo" "$load" "$err_rate" "$cdf" "$cc" 1 0 ar   0 "$default_win_size" 3000 3000 "$buffer_size" drillgroup
                        # run_experiment_group "$topo" "$load" "$err_rate" "$cdf" "$cc" 1 0 noar 0 "$default_win_size" 3000 3000 "$buffer_size" conweave
                        ...
                        # run_experiment_group "$topo" "$load" "$err_rate" "$cdf" "$cc" 1 0 ar   0 "$default_win_size" 3000 3000 "$buffer_size"  adaptive weightadaptive
