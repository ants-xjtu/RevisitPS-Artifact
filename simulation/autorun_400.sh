...
# 腳本會自動對以下所有數組進行全組合測試
TOPOLOGIES=(
    # "leaf_spine_L2_S4_400G_OS1"
    # "leaf_spine_L8_S8_400G_OS2"
    # "leaf_spine_L8_S16_100G_OS1"
    # "leaf_spine_L8_S16_200G_OS1"
    "leaf_spine_L8_S16_400G_OS1"
    # "fat_k8_100G_OS1"
    # "fat_k8_200G_OS1"
    "fat_k8_400G_OS1"
)
...
# 固定實驗參數
RUNTIME="0.05"  # 默認運行時間，會在拓撲循環中根據帶寬自動覆盖 (100G=0.05, 200G=0.04, 400G=0.02)
# RUNTIME="0.2"  # 模擬運行時間 (秒)
FLOWGEN="src"
# FLOWGEN="inter_leaf"
# FLOWGEN="dst"

# ⭐ [NEW] NIC Bandwidth (Gbps) - 由拓扑名自动检测，无需手动修改
NIC_BW="400"  # 默认值，会在拓扑循环中根据拓扑名自动覆盖

# ⭐ [NEW] Buffer Sizes (MB/port) - 支持多个 buffer size 配置
# -1: Tomahawk 动态按带宽自动分配; 0: legacy fallback; >0: 显式指定每端口 MB (支持小数)
BUFFER_SIZES=(
    "1.92"
    # "0.44"
    # "0.32"
...
for topo in "${TOPOLOGIES[@]}"; do
    ...
    # ==================== [新增邏輯] 根據拓撲自動設定帶寬與窗口大小 ====================
    # 根據拓撲名稱中的帶寬標識（100G / 200G / 400G）自動設定 NIC_BW 和 RUNTIME
    if [[ "$topo" == *"100G"* ]]; then
        NIC_BW="100"
        RUNTIME="0.05"
    elif [[ "$topo" == *"200G"* ]]; then
    ...
    # 根據拓撲類型 + 帶寬自動設定默認 win_size
    # fat / three_layer:  100G=156000, 200G=306000, 400G=606000
    # leaf_spine:         100G=104000, 200G=204000, 400G=404000
    if [[ "$topo" == *"fat"* ]] || [[ "$topo" == *"three_layer"* ]]; then
        if [[ "$NIC_BW" == "100" ]]; then
            default_win_size=156000
        elif [[ "$NIC_BW" == "200" ]]; then
            default_win_size=306000
        else
            default_win_size=606000
        fi
    elif [[ "$topo" == *"leaf_spine"* ]] || [[ "$topo" == *"leafspine"* ]]; then
        if [[ "$NIC_BW" == "100" ]]; then
            default_win_size=104000
        elif [[ "$NIC_BW" == "200" ]]; then
            default_win_size=204000
        else
            default_win_size=404000
        fi
    else
        default_win_size=404000
    fi
    cecho "YELLOW" "檢測到拓撲 '${topo}'，NIC 帶寬=${NIC_BW}Gbps，設定默認 win_size 為 ${default_win_size}"
    # =============================================================================
            ...
            for cdf in "${CDFS[@]}"; do
                for cc in "${CCS[@]}"; do
                    for buffer_size in "${BUFFER_SIZES[@]}"; do  # <--- 新增：buffer size 循環
                        ...
                        cecho "BLUE" "=====>> 處理組合: TOPO=[${topo}], LOAD=[${load}], ERR=[${err_rate}], CDF=[${cdf}], CC=[${cc}], BUFFER=[${buffer_size}MB/100G] <<====="
                        ...
                        # #Lossless
                        run_experiment_group "$topo" "$load" "$err_rate" "$cdf" "$cc" 1 0 noar 0 "$default_win_size" 3000 3000 "$buffer_size"  fecmp
                        # run_experiment_group "$topo" "$load" "$err_rate" "$cdf" "$cc" 1 0 ar   0 "$default_win_size" 4000 4000 "$buffer_size"  adaptive
                        ...
                        # # #Lossy
                        run_experiment_group "$topo" "$load" "$err_rate" "$cdf" "$cc" 0 1 noar 0 "$default_win_size" 320 100 "$buffer_size"  fecmp conweave
                        # run_experiment_group "$topo" "$load" "$err_rate" "$cdf" "$cc" 0 1 noar 0 "$default_win_size" 320 100 "$buffer_size"  conga conweave
                        run_experiment_group "$topo" "$load" "$err_rate" "$cdf" "$cc" 0 1 ar   2 "$default_win_size" 320 100 "$buffer_size"  adaptive
                        run_experiment_group "$topo" "$load" "$err_rate" "$cdf" "$cc" 0 1 ar   1 "$default_win_size" 320 100 "$buffer_size"  adaptive
                        run_experiment_group "$topo" "$load" "$err_rate" "$cdf" "$cc" 0 2 ar   0 "$default_win_size" 320 100 "$buffer_size"  adaptive
                        # run_experiment_group "$topo" "$load" "$err_rate" "$cdf" "$cc" 0 1 noar 0 "$default_win_size" 1280 400 "$buffer_size"  fecmp letflow conga conweave
                        # run_experiment_group "$topo" "$load" "$err_rate" "$cdf" "$cc" 0 1 ar   2 "$default_win_size" 1280 400 "$buffer_size"  rps drill adaptive
                        # run_experiment_group "$topo" "$load" "$err_rate" "$cdf" "$cc" 0 1 ar   1 "$default_win_size" 1280 400 "$buffer_size"  rps drill adaptive
