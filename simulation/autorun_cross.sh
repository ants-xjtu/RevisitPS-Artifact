...
CCS=(
    # "dcqcn"           # Regular DCQCN for baseline comparison
    "dcqcn_lane"      # Per-Lane DCQCN with per-lane ECMP
    # "dcqcn_dst"
    # "none"
)

# ⭐ [NEW] Number of lanes for per-lane DCQCN (only used when cc=dcqcn_lane)
LANES_PER_DEST=(
    "2"   # Default: 4 lanes per src-dst pair
    # "2"   # Test with fewer lanes
    # "8"   # Test with more lanes
)
...
run_experiment_group() {
    ...
    local lanes=${14}  # <--- 新增：lanes per destination 参数
    shift 14  # <--- 修改：移除了14个参数
    local lbs=("$@")

    cecho "GREEN" "   -> 提交實驗組: CC=$cc LANES=$lanes PFC=$pfc IRN=$irn ARMODE=$armode TIMEOUT=${timeout_mode} WIN_SIZE=${win_size} RTO_HIGH=${rto_high} RTO_LOW=${rto_low} BUFFER=${buffer_size}MB LBs=${lbs[*]}"
    for lb in "${lbs[@]}"; do
        local log_filename="logs/topo=${topology}_load=${netload}_err=${error_rate}_cdf=${cdf}_cc=${cc}_lanes=${lanes}_pfc=${pfc}_irn=${irn}_armode=${armode}_timeout=${timeout_mode}_win=${win_size}_rtoh=${rto_high}_rtol=${rto_low}_buf=${buffer_size}_lb=${lb}.log"

        run_if_slot_free "$log_filename" python3 run.py \
            --cc "$cc" --lanes "$lanes" --lb "$lb" --pfc "$pfc" --irn "$irn" --armode "$armode" \
            ...
            for cdf in "${CDFS[@]}"; do
                for cc in "${CCS[@]}"; do
                    for buffer_size in "${BUFFER_SIZES[@]}"; do  # <--- 新增：buffer size 循環
                        ...
                        # ==================== [新增邏輯] 根據CC模式決定lanes配置 ====================
                        # 如果是 dcqcn_lane，需要遍歷 lanes 配置；否則使用默認值 4（僅用於兼容性，實際不使用）
                        local lanes_to_test=("4")  # 默認值
                        if [[ "$cc" == "dcqcn_lane" ]]; then
                            lanes_to_test=("${LANES_PER_DEST[@]}")
                        fi

                        for lanes in "${lanes_to_test[@]}"; do
                            if [[ "$cc" == "dcqcn_lane" ]]; then
                                cecho "BLUE" "=====>> 處理組合: TOPO=[${topo}], LOAD=[${load}], ERR=[${err_rate}], CDF=[${cdf}], CC=[${cc}], LANES=[${lanes}], BUFFER=[${buffer_size}MB] <<====="
                            else
                            ...
                            # 9:timeout, 10:winsize, 11:rto_high, 12:rto_low, 13:buffer_size, 14:lanes, 後面跟著所有 lb 模式

                            # big switch test
                            # #Lossy
                            run_experiment_group "$topo" "$load" "$err_rate" "$cdf" "$cc" 0 1 ar   0 "$default_win_size" 320 100 "$buffer_size" "$lanes" adaptive
                            run_experiment_group "$topo" "$load" "$err_rate" "$cdf" "$cc" 0 1 noar   0 "$default_win_size" 320 100 "$buffer_size" "$lanes" fecmp
                            # run_experiment_group "$topo" "$load" "$err_rate" "$cdf" "$cc" 0 1 ar   0 64000 320 100 "$buffer_size" "$lanes" adaptive
                            # run_experiment_group "$topo" "$load" "$err_rate" "$cdf" "$cc" 0 1 noar   0 64000 320 100 "$buffer_size" "$lanes" fecmp
                            # run_experiment_group "$topo" "$load" "$err_rate" "$cdf" "$cc" 0 1 ar   0 32000 320 100 "$buffer_size" "$lanes" adaptive
                            # run_experiment_group "$topo" "$load" "$err_rate" "$cdf" "$cc" 0 1 noar   0 32000 320 100 "$buffer_size" "$lanes" fecmp
                            # run_experiment_group "$topo" "$load" "$err_rate" "$cdf" "$cc" 0 1 ar   0 16000 320 100 "$buffer_size" "$lanes" adaptive
                            # run_experiment_group "$topo" "$load" "$err_rate" "$cdf" "$cc" 0 1 noar   0 16000 320 100 "$buffer_size" "$lanes" fecmp
                            # run_experiment_group "$topo" "$load" "$err_rate" "$cdf" "$cc" 0 1 noar 0 "$default_win_size" 320 100 "$buffer_size" "$lanes" fecmp letflow conga conweave
                            # run_experiment_group "$topo" "$load" "$err_rate" "$cdf" "$cc" 0 1 ar   2 "$default_win_size" 320 100 "$buffer_size" "$lanes" rps drill adaptive
                            # run_experiment_group "$topo" "$load" "$err_rate" "$cdf" "$cc" 0 1 ar   1 "$default_win_size" 320 100 "$buffer_size" "$lanes" rps drill adaptive
                            # run_experiment_group "$topo" "$load" "$err_rate" "$cdf" "$cc" 0 2 ar   0 "$default_win_size" 320 100 "$buffer_size" "$lanes" adaptive
                            # run_experiment_group "$topo" "$load" "$err_rate" "$cdf" "$cc" 0 2 ar   1 "$default_win_size" 320 100 "$buffer_size" "$lanes" adaptive

                            # # #Lossless
                            # run_experiment_group "$topo" "$load" "$err_rate" "$cdf" "$cc" 1 0 noar 0 "$default_win_size" 4000 4000 "$buffer_size" "$lanes" fecmp
                            # run_experiment_group "$topo" "$load" "$err_rate" "$cdf" "$cc" 1 0 noar 0 "$default_win_size" 4000 4000 "$buffer_size" "$lanes" letflow conga conweave
                            # run_experiment_group "$topo" "$load" "$err_rate" "$cdf" "$cc" 1 0 ar   0 "$default_win_size" 4000 4000 "$buffer_size" "$lanes" rps drill adaptive
