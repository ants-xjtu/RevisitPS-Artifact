...
class RandomClient(BfrtGrpcClient):
  def __init__(self, program, hostname, switch_conf, topo_conf, host_conf):
    super(RandomClient, self).__init__(program, hostname, switch_conf, topo_conf, host_conf)
  ...
  def init_downlink_table(self):
    ...
    print("downlink_dev_ports_table:", self.downlink_dev_ports_table)
  ...
  def init_nexthop_id_port_table(self):
    ...
    for port in self.downlink_dev_ports_table[dstToR_node]:
        ...
        self.nexthop_init_id += 1
    print('nexthop_id_port_table:',self.nexthop_id_port_table)
    print('srcToR_to_client_nexthop_id:', self.srcToR_to_client_nexthop_id)
    print('srcToR_to_dstToR_nexthop_id:', self.srcToR_to_dstToR_nexthop_id)
    print('dstToR_to_srcToR_nexthop_id:', self.dstToR_to_srcToR_nexthop_id)
    print('dstToR_to_server_nexthop_id:', self.dstToR_to_server_nexthop_id)
...
def run_random_client(program, args):
  # init grpc client
  random_client = RandomClient(program, args.hostname, args.switches, args.topo, args.hosts)
  random_client.setup(args.bfrt_ip, args.bfrt_port)

  # init port
  port_list = random_client.switch_conf_parser.parse_ports(hostname=args.hostname)
  if port_list is not None:
    random_client.port_controller.add_ports(port_list=port_list)
  else:
    random_client.critical_error("Invalid format detected in {}. Please review the configuration.".format(args.switches))
  
  random_client.init_uplink_table()
  random_client.init_downlink_table()
  random_client.init_switch_id_node_table()
  random_client.init_nexthop_id_port_table()
