...
class NexthopController(NormalController):
  def __init__(self, target, gc, bfrt_info):
    ...
    self.tables = [
      bfrt_info.table_get('pipe.SwitchIngress.get_switch_id'),
    ...
    self.get_switch_id_table = bfrt_info.table_get('pipe.SwitchIngress.get_switch_id')
  ...
  def __add_get_switch_id_entry(self, switch_id, ingress_port):
    self.get_switch_id_table.entry_add(self.target, [
        self.get_switch_id_table.make_key([
            self.gc.KeyTuple('ig_intr_md.ingress_port', ingress_port),
        ])
    ], [
        self.get_switch_id_table.make_data([self.gc.DataTuple('switch_id', switch_id)], 'SwitchIngress.write_switch_id')
    ])
    
  def __add_write_nexthop_id_entry(self, dst_addr, nexthop_id, switch_id):
    self.get_nexthop_id_table.entry_add(self.target, [
        self.get_nexthop_id_table.make_key([
            self.gc.KeyTuple('ig_md.switch_id', switch_id),
  ...
  def add_get_switch_id_entries(self, switch_id, ingress_port_list):
    for ingress_port in ingress_port_list:
      self.__add_get_switch_id_entry(switch_id, ingress_port)
  
  def add_write_nexthop_id_entries(self, dst_addr_list, nexthop_id, switch_id):
    for dst_addr in dst_addr_list:
      self.__add_write_nexthop_id_entry(dst_addr, nexthop_id, switch_id)
