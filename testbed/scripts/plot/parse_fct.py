...
def parse_fct_csv(
    ...
    with open(filename, "r") as f:
        ...
        for row in reader:
            ...
            ideal_fct = 1  # Ideal FCT in ms (your version)
            if ideal_fct <= 0:
                continue
    ...
    for i in range(num_buckets):
        ...
        sorted_slowdowns_in_chunk = sorted(slowdowns_in_chunk)
        p99_slowdown = get_pctl(sorted_slowdowns_in_chunk, 0.99)
        p98_slowdown = get_pctl(sorted_slowdowns_in_chunk, 0.98)
        p97_slowdown = get_pctl(sorted_slowdowns_in_chunk, 0.97)
        p96_slowdown = get_pctl(sorted_slowdowns_in_chunk, 0.96)
        p95_slowdown = get_pctl(sorted_slowdowns_in_chunk, 0.95)

        # bucket size label: max / median / etc.
        size_for_bucket = max(sizes_in_chunk)

        print(
            f"Bucket {i+1}/{num_buckets}: size={size_for_bucket}, avg={avg_slowdown:.3f}, p99={p99_slowdown:.3f}, p98={p98_slowdown:.3f}, p97={p97_slowdown:.3f}, p96={p96_slowdown:.3f}, p95={p95_slowdown:.3f}"
        )

        bucket_data.append({"size": size_for_bucket, "avg": avg_slowdown, "p99": p99_slowdown, "p98": p98_slowdown, "p97": p97_slowdown, "p96": p96_slowdown, "p95": p95_slowdown})
    ...
    return {
        ...
        "p98": [item["p98"] for item in bucket_data],
        "p97": [item["p97"] for item in bucket_data],
        "p96": [item["p96"] for item in bucket_data],
        "p95": [item["p95"] for item in bucket_data],
