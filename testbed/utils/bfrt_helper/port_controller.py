...
class PortController(object):
    ...
    def enable_pfc(self, dev_port):
        self.port_table.entry_mod(self.target, [
            self.port_table.make_key([self.gc.KeyTuple('$DEV_PORT', dev_port)])
        ], [
            self.port_table.make_data([
                self.gc.DataTuple('$TX_PAUSE_FRAME_EN',
                                  bool_val=True),
                self.gc.DataTuple('$RX_PAUSE_FRAME_EN', bool_val=True),
            ])
        ])
        self.port_table.entry_mod(self.target, [
            self.port_table.make_key([self.gc.KeyTuple('$DEV_PORT', dev_port)])
            ...
            self.port_table.make_data([
                self.gc.DataTuple('$TX_PFC_EN_MAP',
                                  0xFF),
                self.gc.DataTuple('$RX_PFC_EN_MAP', 0xFF)
            ])
        ])
        

    def check_active(self, dev_port):
        return dev_port in self.active_ports
