...
def main():
    ...
    traffic_dir = os.path.join(artifact_root, "simulation", "traffic_gen")
    files = {
        "Hadoop": os.path.join(traffic_dir, "FbHdp2015.txt"),
        "RPC": os.path.join(traffic_dir, "Solar2022.txt"),
        "Storage": os.path.join(traffic_dir, "AliStorage2019.txt"),
        "Search": os.path.join(traffic_dir, "WebSearch.txt"),
    }

    # 初始化 CDFPlot
    p_cdf = plot.CDFPlot()
    p_cdf.fig.set_size_inches(8, 4)
    ...
    p_cdf.ax.set_xlabel("Flow Size", fontsize=20)
    p_cdf.ax.set_ylabel("CDF", fontsize=20)
    ...
    p_cdf.ax.xaxis.set_minor_locator(ticker.NullLocator())
    p_cdf.ax.xaxis.set_major_formatter(ticker.FuncFormatter(human_readable_formatter))
    p_cdf.ax.tick_params(axis='both', which='major', labelsize=20)
    p_cdf.ax.tick_params(axis='x', rotation=30)

    # 显示图例和网格
    p_cdf.ax.legend(loc='upper left', fontsize=20)
