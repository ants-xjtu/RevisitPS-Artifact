...
markerstyles = (
    ...
    {'marker': mstyles[7], 'markevery': 1, 'markersize': 12, 'markerfacecolor': 'None'},
    {'marker': mstyles[8], 'markevery': 1, 'markersize': 16, 'markerfacecolor': 'None'},
     
    {'marker': mstyles[6], 'markevery': 1, 'markersize': 14, 'markerfacecolor': 'None'},
   
    
    {'marker': mstyles[9], 'markevery': 1, 'markersize': 16},
    # ---- 新增的几个 ----
    {'marker': mstyles[10], 'markevery': 1, 'markersize': 18, 'markerfacecolor': 'None'},
    {'marker': mstyles[11], 'markevery': 1, 'markersize': 14, 'markerfacecolor': 'white'},
)
...
fillstyles = [
    ...
    {'edgecolor': colors[5], 'fill': False, 'linewidth': 2, 'hatch': 'xx'},
    {'edgecolor': colors[6], 'fill': False, 'linewidth': 2, 'hatch': '++'},
    {'edgecolor': colors[7], 'fill': False, 'linewidth': 2, 'hatch': '||'},
    # ── 新增样式 ──
    {'facecolor': colors[1],  'edgecolor': colors[1],  'linewidth': 2},
    {'facecolor': colors[2],  'edgecolor': colors[2],  'linewidth': 2},
    {'facecolor': colors[3],  'edgecolor': colors[3],  'linewidth': 2},
    {'facecolor': colors[4],  'edgecolor': colors[4],  'linewidth': 2},
    {'facecolor': colors[5],  'edgecolor': colors[5],  'linewidth': 2},
    {'facecolor': colors[6],  'edgecolor': colors[6],  'linewidth': 2},
    {'edgecolor': colors[8],  'fill': False, 'linewidth': 2, 'hatch': '\\\\'},
    {'edgecolor': colors[9],  'fill': False, 'linewidth': 2, 'hatch': '**'},
    {'edgecolor': colors[10], 'fill': False, 'linewidth': 2, 'hatch': 'oo'},
]
...
dashstyles = (
    ...
    {
        'dashes': (1, 3),                  # 点划线（短点 + 空隙）
    },
    {
        'dashes': (3, 1, 1, 1),            # dash-dot（常见样式）
    },
    {
        'dashes': (5, 2, 1, 2),            # 长划线 + 点
    },
    {
        'dashes': (10, 5, 2, 5),           # 超长划线 + 点
    },
)
    ...
    def set_ax_style(self):
        for ax in self.axes:
            ...
            ax.grid(
                ...
                ls=(0, (0.2, 2)), linewidth=2, alpha=0.8,
            )
            # ax.grid(
...
class CDFPlot(LineDashPlot):
    ...
    def set_ax_style(self, axid=0):
        ...
        self.axes[axid].set_ylabel('CDF', fontsize=40)
    ...
    def plot(self, data, cdf=None, axid=0, **kwargs):
        ...
        if 'label' in plot_args:
            self.axes[axid].legend(loc='best')
        self.axes[axid].set_ylim(0, 1)
        self.axes[axid].yaxis.set_major_locator(mpl.ticker.MultipleLocator(0.2))
        return lines
...
class BarPlot(Plot):
    ...
    def get_plot_args(self):
        ...
        '''
        plot_args = super().get_plot_args(0)
        plot_args.update(fillstyles[self.n_groups % len(fillstyles)])
        plot_args['capstyle'] = 'round'
        plot_args['joinstyle'] = 'round'
        return plot_args
...
class BoxPlot(Plot):
    ...
    Attributes:
        n_groups: # of groups
        vals: A list of values for boxplot
        names: the name of each group
    '''
    def __init__(self):
        super().__init__(1)
        self.vals = []
    ...
    def insert_vals(self, x, name=None):
        ''' Insert a group of values into the box
        '''
        self.vals.append(x)
        if name is not None:
    ...
    def plot(self, **kwargs):
        self.ax.boxplot(self.vals, **kwargs)

        # ensure # of names = # of groups
        if len(self.names) < self.n_groups:
            n_residue = self.n_groups - len(self.names)
            self.names = list(self.names).extend(range(
                self.n_groups, n_residue + self.n_groups,
            ))
        elif len(self.names) > self.n_groups:
            self.names = self.names[:self.n_groups]

        self.ax.set_xticks(range(1, self.n_groups + 1), self.names)
